...
class Flowtable_handler(object):
	...
	def add_flow_to_ovs(self,ip1, ip2, mac1, mac2, host1_port, host2_port, ttl):
	...
	def add_flowtable_into_ovs(self,ip1, ip2, mac1, mac2, host1_port, host2_port, ttl):
		...
		br_int.add_flow(table=0,
						...
						nw_src=ip1,
						nw_dst=ip2,
						actions="mod_dl_src:%s,mod_dl_dst:%s,mod_nw_ttl:%s,output:%s" % (mac1,mac2,ttl,host2_port))
		br_int.add_flow(table=0,
						priority=15,
						dl_type=0x0800,
						nw_src=ip2,
						nw_dst=ip1,
						actions="mod_dl_src:%s,mod_dl_dst:%s,mod_nw_ttl:%s,output:%s" % (mac2,mac1,ttl,host1_port))
		print 'add flowtable of  %s to %s successful.' % (ip1,ip2)
