...
class MySqlHelper(object):
	...
	def Get_Dict(self,sql):
		conn = MySQLdb.connect(host = '192.168.1.51',user = 'root', passwd = '123456', port = 3306, db = 'routeflow')
	...
	def Get_network_info_Dict(self,sql):
                ...
                conn = MySQLdb.connect(host = '192.168.1.51',user = 'root', passwd = '123456', port = 3306, db = 'neutron')
                cur = conn.cursor(cursorclass = MySQLdb.cursors.DictCursor)
                reCount = cur.execute(sql)
                data = cur.fetchall()
                cur.close()
